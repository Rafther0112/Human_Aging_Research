...
#%%
moratility_repair_cross_experimentation = np.load('/Users/rafther0112/Documents/GitHub/AGING_RESULTS_SIMULATIONS/Simulacion_cruzado_Recovery_Mortalidad_final_damage_uno.npy', allow_pickle=True)
...
for mortalidad_fija in tqdm(moratility_repair_cross_experimentation):
    ...
    for frailty_curves in mortalidad_fija:
        ...
        for tiempo in np.arange(0,80):
            ...
            for persona in np.arange(0,2000):
...
for mortalidad_fija in tqdm(moratility_repair_cross_experimentation):
    ...
    for frailty_curves in mortalidad_fija:
        ...
        for tiempo in np.arange(0,80):
            frailty_tiempo_fijo = []
            for persona in np.arange(0,1000):
                if frailty_curves[persona, tiempo, 2] == 1:
...
plt.tight_layout()
plt.savefig("Recovery_Damage_Saturation_Shape2.jpg", dpi = 200)
plt.show()
