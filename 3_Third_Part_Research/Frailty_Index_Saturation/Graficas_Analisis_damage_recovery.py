...
#%%
damage_repair_cross_experimentation = np.load('/Users/rafther0112/Documents/GitHub/AGING_RESULTS_SIMULATIONS/Simulacion_cruzado_Damage_Recovery.npy', allow_pickle=True)
# %%
...
#%%
plt.plot(frailty_index_curves_General[-1][-1])
#%%
...
for posicion_damage in range(len(valores_de_damage)):
    ...
    for i in range(len(valores_de_recovery)-posicion_recovery):
        ...
        if -0.01 in frailty_index_curves_General[posicion_damage][posicion_recovery+i]:
            first_zero_index = np.argmax(frailty_index_curves_General[posicion_damage][posicion_recovery+i] == -0.01)
            ax.plot(np.arange(20,100,1)[:first_zero_index],frailty_index_curves_General[posicion_damage][posicion_recovery+i][:first_zero_index], color = color, label = f" Mortality: {valores_de_damage[posicion_damage]}" + "\n" + f"Recovery: {round(valores_de_recovery[posicion_recovery+i],3)}")
        else:
    ...
    plt.savefig(f"Gif_results_damage_recovery/Gif_{posicion_damage}.jpg", dpi = 200)
#%%
#%%
