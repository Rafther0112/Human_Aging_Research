...
array_principal = np.zeros((len(valores_de_nodos),) + (num_cel,tiempo_maximo,3 ))

for posicion_nodo, N_total in enumerate(tqdm(valores_de_nodos)):
    x0 = np.array([0., N_total*initial_condition, 0.])
    array_principal[posicion_nodo] = np.array([Estado_celula(x0,np.arange(0.,tiempo_maximo,1.)) for i in tqdm(range(num_cel))])

    np.save('/Users/rafther0112/Documents/GitHub/AGING_RESULTS_SIMULATIONS/Simulacion_modificacion_numero_nodos.npy', array_principal)
